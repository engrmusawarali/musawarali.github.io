---
layout: course
title: Programming Fundamentals
description: This course covers the foundational aspects of programming, including computational thinking, algorithm design, and program implementation. Students will develop practical skills for solving real-world problems using structured programming techniques.
instructor: Mr. Musawar Ali
year: 2022
term: Fall
location: Department of Computer Science
#time: Mondays and Wednesdays, 2:00-3:30 PM
course_id: Programming Fundamentals
schedule:
  #- week: 1
  #  date: Feb 5
  #  topic: Introduction to Data Science
  #  description: Overview of the data science workflow and key concepts.
  # materials:
  #  - name: Syllabus
  #   url: /assets/pdf/example_pdf.pdf
  # - name: Slides
  #     url: /assets/pdf/example_pdf.pdf
---

## Course Overview

This course provides a comprehensive introduction to programming principles and practices. Students will:

- Become acquainted with problem understanding, modeling, and problem solving

- Understand the fundamental concepts of programming languages

- Learn computation and algorithmic problem-solving techniques

- Implement programs using the C programming language

- Design and implement algorithms to solve real-world problems

- Develop basic skills in program analysis and debugging

## Prerequisites

- None

## Textbooks

- C How to program - 7th Edition
- Problem Solving and programming concepts - 9th Edition

## Grading

- Assignments: 10%
- Project: 10%
- MID - I: 15%
- MID - II: 15%
- Finals: 50%
